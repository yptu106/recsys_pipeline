...
PRE_TRAIN_EPOCHS = 10  # Default pretraining epochs
...
def build_model(model_name, input_dim, d_model, device):
    if model_name == "mlp":
        ...
        return TransformerRanker(input_dim=input_dim, d_model=d_model).to(device)
...
def main():
    ...
    model = build_model(config["model"], input_dim, config.get("d_model", 256), device)
    ...
    if config.get("pretrain", True):
        ...
        trainer.train(
            pretrain_loader, 
            num_epochs=config.get("pretrain_epochs", PRE_TRAIN_EPOCHS), 
            val_dataloader=None, 
            loss_fn=bpr_loss, 
