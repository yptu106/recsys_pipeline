...
from ranker.transformer.pairwise_dataset import PairwiseRankingDataset
...
def train_transformer_ranker(
    ...
    # Initialize model
    input_dim = user_embeddings.shape[1]  # Assuming user and item embeddings have the same dimension
    model = TransformerRanker(input_dim=input_dim).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)

    best_loss = np.inf
    best_epoch = 0
    patience_counter = 0
    best_model_path = pathlib.Path(out_dir) / "transformer_ranker.pth"
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="ranker/models/transformer", help="Output directory for the trained model")
    ...
    # Train the Transformer ranker
    print("Training Transformer ranker...")
    train_transformer_ranker(
        user_embeddings=user_embeddings,
        user_lookup=user_lookup,
        item_embeddings=item_embeddings,
        item_lookup=item_lookup,
